...
import {
  ...
  resolveSignalkApiUrl,
  SIGNALK_API_URL_OPTIONS,
} from "./resolveApiUrl";
...
test("resolveSignalkApiUrl", async (t) => {
  await t.test("trusts and returns a configured URL once it responds to the probe", async () => {
    t.mock.method(
      globalThis,
      "fetch",
      mockFetchOkFor("http://localhost:8080/signalk/v1/unitpreferences/categories"),
    );
  ...
  await t.test(
    "probes SIGNALK_API_URL_OPTIONS in order and returns the first that responds",
    async () => {
      const [, second] = SIGNALK_API_URL_OPTIONS;
      t.mock.method(
        globalThis,
        "fetch",
        mockFetchOkFor(`${second}/signalk/v1/unitpreferences/categories`),
      );
  ...
  await t.test("throws a generic error when none of SIGNALK_API_URL_OPTIONS respond", async () => {
    ...
    await assert.rejects(
      resolveSignalkApiUrl(undefined),
      /could not reach this server's API on any of/,
    );
  });
});
...
test("createApiUrlResolver", async (t) => {
  ...
  await t.test(
    "retries on the next call after a failed resolution instead of caching it",
    async () => {
      let succeed = false;
      t.mock.method(globalThis, "fetch", async () => {
        if (succeed) {
