...
test("fetchJson", async (t) => {
  ...
  await t.test("omits the trailing detail when the error body is empty", async () => {
    ...
    await assert.rejects(
      ...
      /^Error: fetch failed: http:\/\/example\.com \(500 Internal Server Error\)$/,
    );
  });

  await t.test(
    "falls back to an empty body when reading the error response text itself fails",
    async () => {
      t.mock.method(globalThis, "fetch", async () =>
        fakeResponse({
          ok: false,
      ...
      await assert.rejects(
        fetchJson("http://example.com"),
        /^Error: fetch failed: http:\/\/example\.com \(503 Service Unavailable\)$/,
      );
    },
  );
  ...
  await t.test("unwraps a network error whose cause is a plain Error", async () => {
    ...
    await assert.rejects(
      ...
      /fetch failed: http:\/\/localhost:3000 - connect ECONNREFUSED/,
    );
  });
  ...
  await t.test("falls back to an Error cause's code when it has no message", async () => {
    ...
    await assert.rejects(
      fetchJson("http://localhost:3000"),
      /fetch failed: http:\/\/localhost:3000 - ECONNREFUSED/,
    );
  });

  await t.test(
    "joins multiple causes from an AggregateError-shaped cause (IPv6+IPv4 dual-stack failure)",
    async () => {
      t.mock.method(globalThis, "fetch", async () => {
        const err = new TypeError("fetch failed");
  ...
  await t.test(
    "falls back to the raw cause value when it is neither an Error nor an array",
    async () => {
      t.mock.method(globalThis, "fetch", async () => {
        const err = new TypeError("fetch failed");
      ...
      await assert.rejects(
        fetchJson("http://localhost:3000"),
        /fetch failed: http:\/\/localhost:3000 - some non-error cause/,
      );
    },
  );
  ...
  await t.test("falls back to the error message when there is no cause at all", async () => {
    ...
    await assert.rejects(
      fetchJson("http://localhost:3000"),
      /fetch failed: http:\/\/localhost:3000 - totally unexpected failure/,
    );
  });
});
